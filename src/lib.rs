...
fn get_dei(vf:&HcGreekVerbForm, decompose:bool) -> String {
    ...
    if vf.tense == HcTense::Present {
        if vf.mood == HcMood::Indicative {
            s = if decompose {format!("δε {} ει", SEPARATOR) } else { String::from("δεῖ") };
        }
        else if vf.mood == HcMood::Subjunctive {
            s = if decompose {format!("δε {} ῃ", SEPARATOR) } else { String::from("δέῃ") };
        }
        else if vf.mood == HcMood::Optative {
            s = if decompose {format!("δε {} οι", SEPARATOR) } else { String::from("δέοι") };
        }
    }
    else if vf.tense == HcTense::Imperfect {
        if vf.mood == HcMood::Indicative {
            s = if decompose {format!("ε {} δε {} ε", SEPARATOR, SEPARATOR) } else { String::from("ἔδει") };
        }
    }
    else if vf.tense == HcTense::Future {
        if vf.mood == HcMood::Indicative {
            s = if decompose {format!("δεησ {} ει", SEPARATOR) } else { String::from("δεήσει") };
        }
    }
...
fn get_eimi(vf:&HcGreekVerbForm, _decompose:bool) -> String {
...
fn get_keimai(vf:&HcGreekVerbForm, decompose:bool) -> String {
    let mut s = String::from("");
    if vf.tense == HcTense::Present {
        if vf.mood == HcMood::Indicative {
            if vf.person == HcPerson::First {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("κει {} μαι", SEPARATOR) } else { String::from("κεῖμαι") };
                }
                else {
                    s = if decompose {format!("κει {} μεθα", SEPARATOR) } else { String::from("κείμεθα") };
                }
            }
            else if vf.person == HcPerson::Second {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("κει {} σαι", SEPARATOR) } else { String::from("κεῖσαι") };
                }
                else {
                    s = if decompose {format!("κει {} σθε", SEPARATOR) } else { String::from("κεῖσθε") };
                }
            }
            else if vf.person == HcPerson::Third {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("κει {} ται", SEPARATOR) } else { String::from("κεῖται") };
                }
                else {
                    /*fix me?*/
                    s = if decompose {format!("κει {} νται", SEPARATOR) } else { String::from("κεῖνται") };
                }
            }
        }
        else if vf.mood == HcMood::Subjunctive {
            if vf.person == HcPerson::First {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("κε {} ωμαι", SEPARATOR) } else { String::from("κέωμαι") };
                }
                else {
                    s = if decompose {format!("κε {} ωμεθα", SEPARATOR) } else { String::from("κεώμεθα") };
                }
            }
            else if vf.person == HcPerson::Second {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("κε {} ῃ", SEPARATOR) } else { String::from("κέῃ") };
                }
                else {
                    s = if decompose {format!("κε {} ησθε", SEPARATOR) } else { String::from("κέησθε") };
                }
            }
            else if vf.person == HcPerson::Third {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("κε {} ηται", SEPARATOR) } else { String::from("κέηται") };
                }
                else {
                    s = if decompose {format!("κε {} ωνται", SEPARATOR) } else { String::from("κέωνται") };
                }
            }
        }
        else if vf.mood == HcMood::Optative {
            if vf.person == HcPerson::First {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("κε {} οιμην", SEPARATOR) } else { String::from("κεοίμην") };
                }
                else {
                    s = if decompose {format!("κε {} οιμεθα", SEPARATOR) } else { String::from("κεοίμεθα") };
                }
            }
            else if vf.person == HcPerson::Second {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("κε {} οιο", SEPARATOR) } else { String::from("κέοιο") };
                }
                else {
                    s = if decompose {format!("κε {} οισθε", SEPARATOR) } else { String::from("κέοισθε") };
                }
            }
            else if vf.person == HcPerson::Third {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("κε {} οιτο", SEPARATOR) } else { String::from("κέοιτο") };
                }
                else {
                    s = if decompose {format!("κε {} οιντο", SEPARATOR) } else { String::from("κέοιντο") };
                }
            }
        }
        else if vf.mood == HcMood::Imperative {
            if vf.person == HcPerson::First {
                ...
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("κει {} σο", SEPARATOR) } else { String::from("κεῖσο") };
                }
                else {
                    s = if decompose {format!("κει {} σθε", SEPARATOR) } else { String::from("κεῖσθε") };
                }
                ...
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("κει {} σθω", SEPARATOR) } else { String::from("κείσθω") };
                }
                else {
                    s = if decompose {format!("κει {} σθων", SEPARATOR) } else { String::from("κείσθων") };
                }
            }
        }
    }
        ...
        if vf.person == HcPerson::First {
            if vf.number == HcNumber::Singular {
                s = if decompose {format!("ε {} κει {} μην", SEPARATOR, SEPARATOR) } else { String::from("ἐκείμην") };
            }
            else {
                s = if decompose {format!("ε {} κει {} μεθα", SEPARATOR, SEPARATOR) } else { String::from("ἐκείμεθα") };
            }
        }
        else if vf.person == HcPerson::Second {
            if vf.number == HcNumber::Singular {
                s = if decompose {format!("ε {} κει {} σο", SEPARATOR, SEPARATOR) } else { String::from("ἔκεισο") };
            }
            else {
                s = if decompose {format!("ε {} κει {} σθε", SEPARATOR, SEPARATOR) } else { String::from("ἔκεισθε") };
            }
        }
        else if vf.person == HcPerson::Third {
            if vf.number == HcNumber::Singular {
                s = if decompose {format!("ε {} κει {} το", SEPARATOR, SEPARATOR) } else { String::from("ἔκειτο") };
            }
            else {
                s = if decompose {format!("ε {} κει {} ντο", SEPARATOR, SEPARATOR) } else { String::from("ἔκειντο") };
            }
        }
    }
    s
...
fn get_eimi_ibo(vf:&HcGreekVerbForm, decompose:bool) -> String {
    if vf.voice != HcVoice::Active {
        return String::from("");
    }
    let mut s = String::from("");
    if vf.tense == HcTense::Present {
        if vf.mood == HcMood::Indicative {
            if vf.person == HcPerson::First {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("εἰ {} μι", SEPARATOR) } else { String::from("εἶμι") };
                }
                else {
                    s = if decompose {format!("ἰ {} μεν", SEPARATOR) } else { String::from("ἴμεν") };
                }
            }
            else if vf.person == HcPerson::Second {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("εἰ {} ", SEPARATOR) } else { String::from("εἶ") };
                }
                else {
                    s = if decompose {format!("ἰ {} τε", SEPARATOR) } else { String::from("ἴτε") };
                }
            }
            else if vf.person == HcPerson::Third {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("εἰ {} σι(ν)", SEPARATOR) } else { String::from("εἶσι(ν)") };
                }
                else {
                    /*fix me?*/
                    s = if decompose {format!("ἰ {} ᾱσι(ν)", SEPARATOR) } else { String::from("ἴᾱσι(ν)") };
                }
            }
        }
        else if vf.mood == HcMood::Subjunctive {
            if vf.person == HcPerson::First {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("ἰ {} ω", SEPARATOR) } else { String::from("ἴω") };
                }
                else {
                    s = if decompose {format!("ἰ {} ωμεν", SEPARATOR) } else { String::from("ἴωμεν") };
                }
            }
            else if vf.person == HcPerson::Second {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("ἰ {} ῃς", SEPARATOR) } else { String::from("ἴῃς") };
                }
                else {
                    s = if decompose {format!("ἰ {} ητε", SEPARATOR) } else { String::from("ἴητε") };
                }
            }
            else if vf.person == HcPerson::Third {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("ἰ {} ῃ", SEPARATOR) } else { String::from("ἴῃ") };
                }
                else {
                    s = if decompose {format!("ἰ {} ωσι(ν)", SEPARATOR) } else { String::from("ἴωσι(ν)") };
                }
            }
        }
        else if vf.mood == HcMood::Optative {
            if vf.person == HcPerson::First {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("ἰ {} οιμι, ἰ {} οιην", SEPARATOR, SEPARATOR) } else { String::from("ἴοιμι, ἰοίην") };
                }
                else {
                    s = if decompose {format!("ἰ {} οιμεν", SEPARATOR) } else { String::from("ἴοιμεν") };
                }
            }
            else if vf.person == HcPerson::Second {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("ἰ {} οις", SEPARATOR) } else { String::from("ἴοις") };
                }
                else {
                    s = if decompose {format!("ἰ {} οιτε", SEPARATOR) } else { String::from("ἴοιτε") };
                }
            }
            else if vf.person == HcPerson::Third {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("ἰ {} οι", SEPARATOR) } else { String::from("ἴοι") };
                }
                else {
                    s = if decompose {format!("ἰ {} οιεν", SEPARATOR) } else { String::from("ἴοιεν") };
                }
            }
        }
        else if vf.mood == HcMood::Imperative {
            if vf.person == HcPerson::First {
                ...
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("ἰ {} θι", SEPARATOR) } else { String::from("ἴθι") };
                }
                else {
                    s = if decompose {format!("ἰ {} τε", SEPARATOR) } else { String::from("ἴτε") };
                }
                ...
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("ἰ {} τω", SEPARATOR) } else { String::from("ἴτω") };
                }
                else {
                    s = if decompose {format!("ἰ {} οντων", SEPARATOR) } else { String::from("ἰόντων") };
                }
            }
        }
    }
        ...
        if vf.person == HcPerson::First {
            if vf.number == HcNumber::Singular {
                s = if decompose {format!("ᾐ {} α, ᾐ {} ειν", SEPARATOR, SEPARATOR) } else { String::from("ᾖα, ᾔειν") };
            }
            else {
                s = if decompose {format!("ᾐ {} μεν", SEPARATOR) } else { String::from("ᾖμεν") };
            }
        }
        else if vf.person == HcPerson::Second {
            if vf.number == HcNumber::Singular {
                s = if decompose {format!("ᾐ {} εισθα, ᾐ {} εις", SEPARATOR, SEPARATOR) } else { String::from("ᾔεισθα, ᾔεις") };
            }
            else {
                s = if decompose {format!("ᾐ {} τε", SEPARATOR) } else { String::from("ᾖτε") };
            }
        }
        else if vf.person == HcPerson::Third {
            if vf.number == HcNumber::Singular {
                s = if decompose {format!("ᾐ {} ει(ν)", SEPARATOR) } else { String::from("ᾔει(ν)") };
            }
            else {
                s = if decompose {format!("ᾐ {} σαν, ᾐ {} εσαν", SEPARATOR, SEPARATOR) } else { String::from("ᾖσαν, ᾔεσαν") };
            }
        }
    }
    s
...
fn get_oida(vf:&HcGreekVerbForm, decompose:bool) -> String {
    if vf.voice != HcVoice::Active {
        return String::from("");
    }
    let mut s = String::from("");
    if vf.tense == HcTense::Perfect {
        if vf.mood == HcMood::Indicative {
            if vf.person == HcPerson::First {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("οἰδ {} α", SEPARATOR) } else { String::from("οἶδα") };
                }
                else {
                    s = if decompose {format!("ἰσ {} μεν", SEPARATOR) } else { String::from("ἴσμεν") };
                }
            }
            else if vf.person == HcPerson::Second {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("οἰσ {} θα", SEPARATOR) } else { String::from("οἶσθα") };
                }
                else {
                    s = if decompose {format!("ἰσ {} τε", SEPARATOR) } else { String::from("ἴστε") };
                }
            }
            else if vf.person == HcPerson::Third {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("οἰδ {} ε(ν)", SEPARATOR) } else { String::from("οἶδε(ν)") };
                }
                else {
                    /*fix me?*/
                    s = if decompose {format!("ἰσ {} ᾱσι(ν)", SEPARATOR) } else { String::from("ἴσᾱσι(ν)") };
                }
            }
        }
        else if vf.mood == HcMood::Subjunctive {
            if vf.person == HcPerson::First {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("εἰδε {} ω", SEPARATOR) } else { String::from("εἰδῶ") };
                }
                else {
                    s = if decompose {format!("εἰδε {} ωμεν", SEPARATOR) } else { String::from("εἰδῶμεν") };
                }
            }
            else if vf.person == HcPerson::Second {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("εἰδε {} ῃς", SEPARATOR) } else { String::from("εἰδῇς") };
                }
                else {
                    s = if decompose {format!("εἰδε {} ητε", SEPARATOR) } else { String::from("εἰδῆτε") };
                }
            }
            else if vf.person == HcPerson::Third {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("εἰδε {} ῃ", SEPARATOR) } else { String::from("εἰδῇ") };
                }
                else {
                    s = if decompose {format!("εἰδε {} ωσι(ν)", SEPARATOR) } else { String::from("εἰδῶσι(ν)") };
                }
            }
        }
        else if vf.mood == HcMood::Optative {
            if vf.person == HcPerson::First {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("εἰδ {} ειην", SEPARATOR) } else { String::from("εἰδείην") };
                }
                else {
                    s = if decompose {format!("εἰδ {} ειμεν, εἰδ {} ειημεν", SEPARATOR, SEPARATOR) } else { String::from("εἰδεῖμεν, εἰδείημεν") };
                }
            }
            else if vf.person == HcPerson::Second {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("εἰδ {} ειης", SEPARATOR) } else { String::from("εἰδείης") };
                }
                else {
                    s = if decompose {format!("εἰδ {} ειτε, εἰδ {} ειητε", SEPARATOR, SEPARATOR) } else { String::from("εἰδεῖτε, εἰδείητε") };
                }
            }
            else if vf.person == HcPerson::Third {
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("εἰδ {} ειη", SEPARATOR) } else { String::from("εἰδείη") };
                }
                else {
                    s = if decompose {format!("εἰδ {} ειεν, εἰδ {} ειησαν", SEPARATOR, SEPARATOR) } else { String::from("εἰδεῖεν, εἰδείησαν") };
                }
            }
        }
        else if vf.mood == HcMood::Imperative {
            if vf.person == HcPerson::First {
                ...
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("ἰσ {} θι", SEPARATOR) } else { String::from("ἴσθι") };
                }
                else {
                    s = if decompose {format!("ἰσ {} τε", SEPARATOR) } else { String::from("ἴστε") };
                }
                ...
                if vf.number == HcNumber::Singular {
                    s = if decompose {format!("ἰσ {} τω", SEPARATOR) } else { String::from("ἴστω") };
                }
                else {
                    s = if decompose {format!("ἰσ {} των", SEPARATOR) } else { String::from("ἴστων") };
                }
            }
        }
    }
        ...
        if vf.person == HcPerson::First {
            if vf.number == HcNumber::Singular {
                s = if decompose {format!("ε {} εἰδ {} η, ε {} εἰδ {} ειν", SEPARATOR, SEPARATOR, SEPARATOR, SEPARATOR) } else { String::from("ᾔδη, ᾔδειν") };
            }
            else {
                s = if decompose {format!("ε {} εἰσ {} μεν, ε {} εἰδ {} εμεν", SEPARATOR, SEPARATOR, SEPARATOR, SEPARATOR) } else { String::from("ᾖσμεν, ᾔδεμεν") };
            }
        }
        else if vf.person == HcPerson::Second {
            if vf.number == HcNumber::Singular {
                s = if decompose {format!("ε {} εἰδ {} ησθα, ε {} εἰδ {} εις", SEPARATOR, SEPARATOR, SEPARATOR, SEPARATOR) } else { String::from("ᾔδησθα, ᾔδεις") };
            }
            else {
                s = if decompose {format!("ε {} εἰσ {} τε, ε {} εἰδ {} ετε", SEPARATOR, SEPARATOR, SEPARATOR, SEPARATOR) } else { String::from("ᾖστε, ᾔδετε") };
            }
        }
        else if vf.person == HcPerson::Third {
            if vf.number == HcNumber::Singular {
                s = if decompose {format!("ε {} εἰδ {} ει(ν)", SEPARATOR, SEPARATOR) } else { String::from("ᾔδει(ν)") };
            }
            else {
                s = if decompose {format!("ε {} εἰσ {} αν, ε {} εἰδ {} εσαν", SEPARATOR, SEPARATOR, SEPARATOR, SEPARATOR) } else { String::from("ᾖσαν, ᾔδεσαν") };
            }
        }
    }
    s
...
impl HcVerbForms for HcGreekVerbForm {
    ...
    fn get_form(&self, decompose:bool) -> Result<Vec<Step>, HcFormError> {
        ...
        if self.verb.pps[0] == "δεῖ" {          
            ...
            if self.tense == HcTense::Present || self.tense == HcTense::Imperfect {
                let fff = get_keimai(self, decompose);
                if fff.is_empty() {
                    return Err(HcFormError::IllegalForm);
                }
                steps.push(Step{form:fff, explanation:String::from("def")});
            ...
            if self.tense == HcTense::Present || self.tense == HcTense::Imperfect {
                let fff = get_eimi_ibo(self, decompose);
                if fff.is_empty() {
                    return Err(HcFormError::IllegalForm);
                }
                steps.push(Step{form:fff, explanation:String::from("def")});
        ...
        if self.verb.pps[0] == "ἀποθνῄσκω" && decompose && (self.tense == HcTense::Perfect || self.tense == HcTense::Pluperfect) {
            ...
            else if !add_ending_collector.is_empty() && add_ending_collector[0] == "τεθνηκ ‐ ᾱσι(ν)" {
                let alt = String::from("τεθν ‐ ᾱσι(ν)");
                add_ending_collector.push(alt);
            }
            else if !add_ending_collector.is_empty() && add_ending_collector[0] == "ε ‐ τεθνηκ ‐ εσαν" {
                let alt = String::from("ε ‐ τεθν ‐ ασαν");
                add_ending_collector.push(alt);
            }
        }
        
        if add_ending_collector.is_empty() { //this catches meanesthn in aorist middle, etc.; fix me? should be better way to catch these
        ...
        if !decompose {
            ...
            if self.verb.pps[0] == "ἀποθνῄσκω" && (self.tense == HcTense::Perfect || self.tense == HcTense::Pluperfect) {
                ...
                else if !add_accent_collector.is_empty() && add_accent_collector[0] == "τεθνήκᾱσι(ν)" {
                    let alt = String::from("τεθνᾶσι(ν)");
                    add_accent_collector.push(alt);
                }
                else if !add_accent_collector.is_empty() && add_accent_collector[0] == "ἐτεθνήκεσαν" {
